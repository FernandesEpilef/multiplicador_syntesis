module multiplier (
    ...
    // remove o stado 'MUL'
    typedef enum logic [2:0] {IDLE, MULT_0, MULT_1, MULT_2, MULT_3, 
    RESULT} state_t;
    ...
    assign sinal_a = (operation == 2'b01) || (operation == 2'b10); // MULHU ou
    // MULHSU
    assign sinal_b = (operation == 2'b01); // MULH
    ...
    always_comb begin
        ...
        case (state)
            IDLE: begin
                in_ready_o = 1'b1;
                if (in_valid_i) begin
                    // mudança aqui: todas as operações passarão pelo mesmo
                    //caminho
                    next_state = MULT_0;
                end
            end
            ...
            MULT_1: begin
                // P1 = A_H (sinal/sem) * B_L (sem sinal)
                mul_x = {(sinal_a & register_a[31]), register_a[31:16]};
                mul_y = {1'b0, register_b[15:0]};

                // soma deslocando 16 bits
                next_acumulador = acumulador + {{14{mul_out[33]}}, mul_out, 
                16'd0};
                next_state = MULT_2;
            end
            ...
            MULT_2: begin
                ...
                // desloca 16 bits e extende sinal
                next_acumulador = acumulador + { {14{mul_out[33]}}, mul_out,
                16'd0};
                next_state = MULT_3;
            end
            ...
            MULT_3: begin
                ...
                // 3. Mux para selecionar o resultado final
                if (operation == 2'b00) begin
                    next_resultado = next_acumulador[31:0];  // MUL usa os 32
                    //bits menos significativos
                end else begin
                    next_resultado = next_acumulador[63:32]; // MULH, MULHSU, MULHU
                    //usam a parte alta
                end
